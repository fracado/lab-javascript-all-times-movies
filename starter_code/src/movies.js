...
// Get the average of Drama Movies rate
function dramaMoviesRate(movies) {
  ...
  if (dramas.length === 0) {
    ...
    var sum = dramas.reduce(function(acc,dramas) {
      if (dramas.rate === "") {
        return acc;
      } else {
        return acc + parseFloat(dramas.rate);
      }
    }, 0)
...
function howManyMovies(movies) {
  if (movies.length === 0) {
    ...
    var noOfDrama = movies.filter(function(movie) {
      return (
        movie.genre.includes("Drama") && movie.director === "Steven Spielberg"
      );
    }).length;
    return "Steven Spielberg directed " + noOfDrama + " drama movies!";
  }
}
...
function orderAlphabetically(movies) {
  ...
  console.log(movies);
  var res = [];
  for (var i = 0; i < Math.min(20, movies.length); i++) {
...
function bestYearAvg(movies) {
  ...
  if (movies.length === 0) {
    ...
    avg.push({
      year: movies[0].year,
      avgRate: parseFloat(movies[0].rate)
    });
  } else {
    ...
    avg.sort(function(y1, y2) {
      if (y1.avgRate > y2.avgRate) {
        return -1;
      } else if (y1.avgRate < y2.avgRate) {
        return 1;
      } else {
        if (y1.year < y2.year) {
          return -1;
